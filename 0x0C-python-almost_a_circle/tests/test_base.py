...
class TestBase(unittest.TestCase):
    ...
    def test_load_from_file(self):
        ...
        r200 = Rectangle(2, 4)
        list_entry = [r100, r200]
        Rectangle.save_to_file(list_entry)
        list_out = Rectangle.load_from_file()
        self.assertNotEqual(id(list_entry[0]), id(list_out[0]))
        self.assertEqual(str(list_entry[1]), '[Rectangle] (4) 0/0 - 2/4')

        s100 = Square(5)
        s200 = Square(7, 9, 1)
        list_entry2 = [s100, s200]
        Square.save_to_file(list_entry2)
        list_out = Square.load_from_file()
        self.assertNotEqual(id(list_entry2[0]), id(list_out[0]))
        self.assertEqual(str(list_entry2[0]), '[Square] (7) 0/0 - 5')
        self.assertEqual(str(list_entry2[1]), '[Square] (8) 9/1 - 7')
